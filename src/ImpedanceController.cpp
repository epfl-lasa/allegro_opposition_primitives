...
void ImpedanceController::Resize_e() {
  ...
  for (int i = 0; i<nJoints; i++)
    e_jointEffortMask.push_back(Eigen::MatrixXd::Identity(nJoints, nJoints));
}
...
void ImpedanceController::SetJointEffortMask(int joint_id, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> mask) {
  e_jointEffortMask[joint_id] = mask;
}
...
void ImpedanceController::Update_e(const KDL::JntArray _jnts) {
  ...
  //cerr << "[Imp] axis :" << e_angleaxis.axis()[0] << " " << e_angleaxis.axis()[1] << " " <<  e_angleaxis.axis()[2] << " angle: " << e_angleaxis.angle() << endl;
...
//  e_control_torque = e_jacobianT * e_control_wrench;
  for (int i=0; i<nJoints; i++)
      e_control_torque(i,0) = e_jacobianT.row(i) * e_jointEffortMask[i] * e_control_wrench;
